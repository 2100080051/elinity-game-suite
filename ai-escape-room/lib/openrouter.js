...
export function extractJson(s) {
  ...
  const pattern = /{[\s\S]*?"room_state"[\s\S]*?}/m; // generic catch-all for state blobs
  const titleDescPattern = /{[\s\S]*?"title"\s*:\s*"[^"]+"[\s\S]*?"description"\s*:\s*"[^"]+"[\s\S]*?}/m;
  const match = s.match(titleDescPattern) || s.match(pattern);
  if (match) {
  ...
  if (i >= 0 && j >= 0 && j > i) {
    return JSON.parse(s.slice(i, j + 1).replace(/\r?\n/g, ' '));
  }
