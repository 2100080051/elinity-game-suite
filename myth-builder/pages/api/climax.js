...
const SYSTEM = `As ElinityAI narrator, deliver the mythic climax and moral. OUTPUT JSON: { climax, moral }. No extra text.`;

export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  try {
    const { world, characters = [], rounds = [] } = req.body || {};
    const context = `World: ${world?.name||'Unknown'} (${world?.theme||''})\nCharacters: ${characters.map(c=> c.type+': '+c.text).join('; ')}\nStory: ${rounds.map(r=> `[${r.round}] ${r.text}`).join(' ')}`;
