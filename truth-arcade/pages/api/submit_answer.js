...
export default async function handler(req, res){
  ...
  const {game_state, answer} = req.body
  if(!game_state || !answer){
    return res.status(400).json({error:'game_state and answer required'})
  }
  ...
  if(!apiKey){
    ...
    return res.json({
      game_state,
      comment: "Great answer! You earned a token! 🪙",
      next_question: game_state.round > 10 ? null : `${currentPlayer}, your turn! What's something you've never told anyone?`,
    ...
    try{
      const apiRes = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
        ...
        messages: [
          ...
          {role: 'user', content: `Player answered: "${answer}"\n\nGive a brief, witty comment on their answer (1-2 sentences). Acknowledge they earned a token.`}
        ],
        temperature: 0.85,
        max_tokens: 150
      }, {
  ...
  // Fallbacks
  if(!comment) comment = "Honest answer! You earned a token! 🪙"
  if(!nextQuestion && game_state.round <= 10) nextQuestion = `${currentPlayer}, what's something you're proud of but rarely talk about?`
